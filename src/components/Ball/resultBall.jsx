...
export default function ResultBalls({ numbers, result = [] }) {
  ...
  return (
    <div className="flex gap-1 justify-center">
      {nums.map((n, idx) => {
        const matched = result.includes(Number(n));
        ...
        return (
          <div key={idx} className="relative w-[30px] h-[30px]">
            <div
              ...
              tabIndex={0}
              style={{
                background: matched ? "radial-gradient(100% 60% at 30% 25%, rgba(236, 236, 228, 0.3), rgba(216, 209, 16, 0.91))" :
                  "radial-gradient(100% 60% at 30% 25%, rgba(255,255,255,0.3), rgba(0,0,0,0.2))",
              }}
            >
              {n === null || n === undefined || n === "" ? "-" : n < 10 ? `0${n}` : n}
            </div>

            <div
              className="absolute left-1/2 transform -translate-x-1/2 bottom-[-6px] 
              w-3/5 h-2 rounded-full filter blur-md opacity-40 
              bg-gradient-to-r from-black/30 via-black/15 to-black/15"
            />
          </div>
        );
      })}
